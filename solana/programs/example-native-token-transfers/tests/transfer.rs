...
async fn test_transfer(ctx: &mut ProgramTestContext, test_data: &TestData, mode: Mode) {
    ...
    let sequence: Sequence = ctx.get_account_data_anchor(GoodNTT {}.sequence()).await;
    ...
    .await
    .unwrap();
    transfer(&GoodNTT {}, accs, args, mode)
    ...
    let wh_message = GoodNTT {}.wormhole_message(&outbox_item.pubkey());
    ...
    let next_sequence: Sequence = ctx.get_account_data_anchor(GoodNTT {}.sequence()).await;
...
async fn test_burn_mode_burns_tokens() {
    ...
    .await
    .unwrap();
    transfer(&GoodNTT {}, accs, args, Mode::Burning)
...
async fn locking_mode_locks_tokens() {
    ...
    let custody_account_before: TokenAccount = ctx
        .get_account_data_anchor(GoodNTT {}.custody(&test_data.mint))
        .await;
...
async fn test_transfer_wrong_mode() {
    ...
    let err = transfer(&GoodNTT {}, accs.clone(), args.clone(), Mode::Locking)
        .submit_with_signers(&[&test_data.user, &outbox_item], &mut ctx)
        .await
        .unwrap_err();
