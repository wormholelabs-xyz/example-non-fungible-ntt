...
async fn test_receive() {
    ...
    spl_token::instruction::transfer_checked(
        ...
        &test_data.mint,
        &GoodNTT {}.custody(&test_data.mint),
    ...
    let msg = make_transfer_message(&GoodNTT {}, 0, 1000, &recipient.pubkey());
    ...
    release_inbound_unlock(
        ...
        ReleaseInbound {
            payer: ctx.payer.pubkey(),
            inbox_item: GoodNTT {}.inbox_item(OTHER_CHAIN, msg.ntt_manager_payload.clone()),
            mint: test_data.mint,
...
async fn test_double_receive() {
    ...
    let err = receive_message(
        &GoodNTT {},
        init_receive_message_accs(&mut ctx, vaa1, OTHER_CHAIN, 0),
    )
    .submit(&mut ctx)
    .await
    .unwrap_err();
...
async fn test_wrong_recipient_ntt_manager() {
    ...
    let mut msg = make_transfer_message(&GoodNTT {}, 0, 1000, &recipient.pubkey());
